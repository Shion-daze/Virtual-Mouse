...
y_hand_previous = 0
y_hand_initial = None  # スクロールの初期位置を保存
current_mode = "cursor"
    ...
    if hands:
        ...
        if click_mode_counter == 0:
            if is_index_finger_up(landmark):
                ...
                current_mode = "scroll"
                y_hand_initial = None  # スクロールの初期位置をリセット
        ...
        if current_mode == "cursor":
            ...
            screen_x = int(index_landmark.x * SCREEN_WIDTH * SENSITIVITY)
            screen_y = int(index_landmark.y * SCREEN_HEIGHT * SENSITIVITY)
            ...
            if y_hand_initial is None:
                y_hand_initial = y_hand_current  # スクロール初期位置を設定

            scroll_speed = (y_hand_previous - y_hand_current) * SCROLL_SENSITIVITY

            if abs(y_hand_current - y_hand_initial) > SCROLL_THRESHOLD:
